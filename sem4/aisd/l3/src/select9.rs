...
fn partition_median_of_medians(arr: &mut [usize], p: usize, q: usize, comparisons: &mut usize, swaps: &mut usize) -> usize {
    ...
    let mut start = p;
    while start <= q {
        let end = std::cmp::min(start + 8, q);
        let median = median_of_nine(&mut arr[start..=end], comparisons, swaps);
        medians.push(median);
        start += 5;
    }

    let medians_len = medians.len();
    let median_of_medians = if medians.len() <= 9 {
        median_of_nine(&mut medians, comparisons, swaps) 
    } else {
...
fn median_of_nine(arr: &mut [usize], comparisons: &mut usize, swaps: &mut usize) -> usize {
